    ...
    const std::vector<double>& growthdirection)
{
  ...
  for (unsigned i = 0; i < growthdirection.size(); ++i)
  {
    ...
    {
      growth_dir_tensor_(i, j) = invquadrgrowthdirvecnorm * growthdirection[i] * growthdirection[j];
    }
  }
}
    ...
    Core::LinAlg::Matrix<3, 3>& iFinM)
{
  ...
  FOUR_C_ASSERT_ALWAYS(growthfactor > 0.0,
      "Determinante of growth must not become negative, please check your input file!");
