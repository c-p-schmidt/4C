    ...
    int num_components)
{
  ...
  const auto existing = std::find_if(written_arrays.begin(), written_arrays.end(),
      [&name](const VtuArrayMetadata& array) { return array.name == name; });
  if (existing == written_arrays.end()) written_arrays.push_back({name, type, num_components});
}
    ...
    const std::vector<VtuArrayMetadata>& local_arrays) const
{
  ...
  for (int rank = 0; rank < numproc; ++rank)
  {
    ...
    const std::size_t end = static_cast<std::size_t>(sizes[rank]);
    while (position < end)
    {
      ...
      const auto existing = std::find_if(union_arrays.begin(), union_arrays.end(),
          [&name](const VtuArrayMetadata& array) { return array.name == name; });
      if (existing == union_arrays.end()) union_arrays.push_back({name, type, num_components});
    }
  }
  return union_arrays;
}
