    ...
    const std::map<std::string, Core::IO::visualization_vector_type_variant>& field_data_map)
{
  ...
  for (const auto& [field_name, field_data] : field_data_map)
    write_field_data_array(field_name, std::get<std::vector<double>>(field_data));
...
void VtuWriter::write_vtk_header_master_file(const std::string& byteorder)
{
  ...
  currentmasterout_ << "-->\n";
  currentmasterout_ << "<VTKFile type=\"P" << this->writer_string() << R"(" version="0.1")";
...
void VtuWriter::write_vtk_header_this_processor(const std::string& byteorder)
{
  ...
  currentout_ << "-->\n";
  currentout_ << "<VTKFile type=\"" << this->writer_string() << R"(" version="0.1")";
...
void VtuWriter::write_field_data_array(const std::string& name, const std::vector<T>& field_data)
{
  ...
  currentout_ << "\" Name=\"";
  currentout_ << name;
  currentout_ << R"(" NumberOfTuples="1")";
...
void VtuWriter::write_vtk_footer_master_file()
{
  ...
  for (const auto& ppiecetag : ppiecetags) currentmasterout_ << "    " << ppiecetag << "\n";
